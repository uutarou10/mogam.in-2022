...
const Articles: NextPage<{ media: Medium[] }> = ({ media }) => {
  ...
  const tabSelectorItems: TabSelectorItem[] = React.useMemo(() => {
    return [
      {
        label: 'All',
        isActive: activeMediumName === 'All',
        onClick: () => setActiveMediumName('All')
      },
  ...
  const activeArticles: Article[] = React.useMemo(() => {
    if (activeMediumName === 'All') {
      return media.reduce(
        (prev, medium) => [...prev, ...medium.articles],
        [] as Article[]
      )
    }
    ...
    return (
      media
        .find((medium) => medium.name === activeMediumName)
    ...
    return {
      props: {
        media: media.map((medium) => ({
          ...medium,
          articles: medium.articles.map((article) => ({
            ...article,
